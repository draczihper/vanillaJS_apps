...
const = renderNewQoute = async () => {
    ...
    // Array of character (chars) in quote
...
userInput.addEventListener("input", () => {
    ...
    //Array from user input chars
    let userInputChars = userInput.value.split("");
    // Loop through each character in quote
    quoteChars.forEach((char, index) => {
        // Check input characters with quote characters
        if(char.innerText == userInputChars[index]) {
            char.classList.add("success");
        }
        // Else if user hasn't add anything or a backspace
            ...
            // If a user entered a wrong character
            if(!char.classList.contains("fail")) {
                // Incremeant and display mistakes
                mistakes++;
        ...
        //Return true of all character are correct
        let check = qouteChars.every((element) => {
            return element.classList.contain("success");
        });
...
function updateTimer() {
    if(time == 0) {
        //End the test if time reaches 0s
        displayResult();
    } else {
...
// CODE FROM GITHUB TO CHECK WHAT IS WRONG
